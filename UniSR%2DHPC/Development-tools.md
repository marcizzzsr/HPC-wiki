...
> The full collection of these tools can be found [here](https://github.com/AI-UniSR/cluster-entrypoints) along with custom documentation files for each tool implemented. You can find a stable version of the repository in the cluster at `/mnt/data/unisr-data/entrypoints`
...
```bash
./wrapper.sh <host-workdir> [--bind <host:container> ...]
```
...
```bash
./wrapper.sh ~/projects/mycode --bind /mnt/data/datasets:/workspace/datasets --bind /mnt/tools:/workspace/tools
```
...
On the first run, the script will require you to log in with a GitHub or Microsoft account to authenticate the tunnel. This is a one-time operation, as your login will be persisted for future sessions.
...
2.  A full-featured VSCode editor will open in your web browser, connected to your HPC environment.

**B) From a Local VSCode Application (Recommended)**
...
*   To modify the Apptainer image path or other container settings, you will need to edit the corresponding `job.sh` file.
